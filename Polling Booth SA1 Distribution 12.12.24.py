...
SA1_By_PP = SA1_By_PP.rename(columns={'ccd_id': 'SA1_CODE21'})
#print(SA1_By_PP)
...
def create_Div_First_Prefs_PP_dict(First_Prefs_by_PP_Complete):
...
Div_First_Prefs_PP_dict = create_Div_First_Prefs_PP_dict(First_Prefs_by_PP_Complete)
...
def convert_to_wide_format(df):
    pivot_df = df.pivot_table(index=['pp_id','pp_nm','div_nm'], 
                            columns=['cand_id', 'PartyAb'], 
                            values='votes', 
                            aggfunc='first',
...
def convert_dict_to_wide_format(dict):
    converted_dict = {}
    for key in dict:
        converted_dict[key] = convert_to_wide_format(dict[key])

    return converted_dict



print(convert_to_wide_format(Div_First_Prefs_PP_dict["Kooyong"]))


Div_First_Prefs_PP_dict_wide = convert_dict_to_wide_format(Div_First_Prefs_PP_dict)
...
SA1_By_PP_Complete = SA1_By_PP.groupby(["div_nm","SA1_CODE21","pp_id"], as_index=False)["votes"].sum()

# check if correct for specific SA1s - YES!
#print(SA1_By_PP.loc[SA1_By_PP["SA1_CODE21"]==7103112,].to_string())
#print(SA1_By_PP_Complete.loc[SA1_By_PP_Complete["SA1_CODE21"]==7103112,].to_string())
...
def Electorate_SA1_PB(SA1_By_PP, DivName, SA1_CODE21):
    ...
    Electorate_PB = PB_combined.loc[PB_combined["DivName"]==DivName,]
    Electorate_SA1_PB = SA1_By_PP.loc[SA1_By_PP["SA1_CODE21"]==SA1_CODE21,]
...
def Goldstein_specific():
    ...
    #print(SA1_By_PP.loc[SA1_By_PP["SA1_CODE21"]==2117417,])
    SA1_Lat = -37.9138684
    SA1_Long = 145.0263502
    Goldstein_PB = PB_combined.loc[PB_combined["DivName"]=="Goldstein",]
    Goldstein_PB_2117417 = SA1_By_PP.loc[SA1_By_PP["SA1_CODE21"]==2117417,]
