...
# 5. Equip PBs, PPVCs with Coordinates using PB_combined? # Perhaps use new download for PBs Lat
# 6. Absorb EAVs, APPPs, RMTs into 1 categoery in SA1s df
# 7. Compare total votes with SA1s vs Candidates to see gaps
...
def label_PB_PPVC(First_Prefs_by_PP_Incomplete):
    ...
    #print(list(PPVC_pp_ids), "number of unique pp_ids that are PPVCs: ",len(PPVC_pp_ids))
    print(list(PB_pp_ids), "number of unique pp_ids that are PPVCs: ",len(PB_pp_ids))

    #First_Prefs_by_PP_Incomplete.loc[First_Prefs_by_PP_Incomplete["pp_nm"].str.endswith("PPVC"),"Booth_type"] = "PPVC"


    return PB_pp_ids # First_Prefs_by_PP_Complete

PB_pp_ids = label_PB_PPVC(First_Prefs_by_PP_Incomplete)
...
j = 0
for id in PB_pp_ids:
    if id not in PB_combined_PB_pp_ids:
        print(id)
        j+= 1
print("Count: ", j)
...
Polling_places_repository = pd.read_csv("PollingPlaces2022Repository.csv", index_col=None)




def Candidate_Votes_by_PP():
