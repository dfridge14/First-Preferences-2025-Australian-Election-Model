...
def format_old_new_correspondence(data_year, abolished_divs):
    old_year = str(int(data_year) - 4)
    new_year = str(int(data_year) - 1)
    
    if old_year == '2015':
        Correspondence_CED_old_new = pd.read_csv(f'CG_CED_{old_year}_CED_{new_year}.csv', index_col=None).rename(columns={f'CED_NAME_{old_year}':f'div_nm_{old_year}',f'CED_NAME_{new_year}':f'div_nm_{new_year}'})
...
def reverse_new_old_correspondence(Correspondence_df, data_year):
    ...
    total_per_new = Correspondence_df.groupby(f'div_nm_{new_year}')['RATIO_FROM_TO'].transform('sum')
    Correspondence_df['proportion'] = Correspondence_df['RATIO_FROM_TO'] / total_per_new

    Correspondence_df = Correspondence_df[[f'div_nm_{new_year}',f'div_nm_{old_year}','proportion']].rename(columns = {f'div_nm_{new_year}':'new_div', f'div_nm_{old_year}':'old_div'}).sort_values(by='new_div')

    import pdb;pdb.set_trace()

    Correspondence_df.to_csv(f'Correspondence_CED_{new_year}_{old_year}_Reversed.csv', index=False)
...
def reverse_new_old_correspondence(Correspondence_df, old_year, new_year):
    # matches pretty closely with proportions_moved file
